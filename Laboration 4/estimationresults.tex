...
\label{realdatago}
To find the accuracy of the subjective estimation and the Goel-Okumoto method the real values of $N_{future}$ and $N_{tot}$ were found by using a file, \code{godata2.txt}, containing the failure count of 31 more weeks. 
...
In this excersice a file, \code{jmdata1.txt}, containing the failures found in a system as well as the time between them will be used.
...
To use the Jalinski-Moranda method, described in section~\ref{methods}, a program, \code{jm.m}, were used to calculate the variables $N=226.5$ and $\phi=1.18\times10^{-4}$. Since Jelinski-Moranda estimates the intensity of failures $N_{future}$ cannot be estimated using this method.
